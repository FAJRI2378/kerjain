  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl space-y-3">
      ...
      <div class="flex items-baseline justify-between">
        <span class="text-3xl font-black text-white">{formatNumber(stats.totalUsers)}</span>
    ...
    <div class="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl space-y-3">
      ...
      <div class="flex items-baseline justify-between">
        <span class="text-xl font-black text-white">{formatRupiah(stats.monthlyRevenue)}</span>
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between p-4 bg-slate-950/70 border border-slate-800/80 rounded-xl hover:border-slate-700 transition gap-4">
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-slate-500 font-medium">{job.category?.name ?? '-'}</span>
                {#if job.created_at}<span class="text-xs text-slate-600">• {new Date(job.created_at).toLocaleString('id-ID')}</span>{/if}
              </div>
              <p class="font-bold text-white text-sm">{job.title}</p>
              <p class="text-xs text-slate-400">Diposting oleh: <span class="text-slate-200 font-medium">{job.owner?.name ?? '-'}</span></p>
            </div>
