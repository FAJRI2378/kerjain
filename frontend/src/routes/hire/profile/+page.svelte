<script>
  ...
  import BusinessCard from '$lib/components/profile/business-card.svelte';
...
<div class="profile-page font-sans">
  <div class="page-header">
    <h1 class="page-title">Profil & Verifikasi Usaha</h1>
    <p class="page-sub">Kelola akun, profil usaha, dan status verifikasi UMKM Anda.</p>
  </div>
  ...
  {:else}
    <div class="grid">
      <VerificationStatusCard role="hirer" isVerified={!!auth.user?.is_verified} {verification} />
      <AccountCard />
      <BusinessCard />
      <ChangePasswordCard />
    </div>
