<script>
  ...
  const contacts = $derived(
    jobs
      .filter((j) => j.worker?.name && waLink(j.worker.phone))
      .map((j) => ({
        jobId: j.id,
        jobTitle: j.title,
        status: j.status,
...
<svelte:head>
  <title>Kontak Freelancer - UMKM Portal</title>
</svelte:head>
...
<div class="kontak-page font-sans">
  <div class="page-header">
    <div>
      ...
      <h1 class="page-title">Kontak Freelancer</h1>
      <p class="page-sub">Hubungi freelancer yang sedang mengerjakan tugas Anda langsung melalui WhatsApp.</p>
    </div>
  ...
  {:else if contacts.length === 0}
    <div class="state-box">
      ...
      <p class="state-title">Belum ada freelancer aktif</p>
      <p class="state-sub">Kontak freelancer akan muncul di sini setelah tugas Anda memiliki pekerja yang ditugaskan.</p>
      <a href="/hire/jobs" class="btn-primary">Lihat Tugas Saya</a>
    </div>
  {:else}
    <div class="contact-list">
      {#each contacts as c (c.jobId)}
        <div class="contact-row">
          <div class="contact-avatar">{c.name.charAt(0)}</div>
          <div class="contact-info">
            <h3 class="contact-name">{c.name}</h3>
            <p class="contact-desc">📋 {c.jobTitle}</p>
          ...
          <a
            href={waLink(c.phone, `Halo ${c.name}, saya dari Kerjain ingin membahas pekerjaan \"${c.jobTitle}\" (#${c.jobId}).`)}
