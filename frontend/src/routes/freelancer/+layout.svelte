    ...
    <div class="pt-4 border-t border-slate-800/80 space-y-3">
      ...
      <div class="flex items-center gap-3 px-2 py-2 rounded-xl bg-slate-950/60 border border-slate-800/50">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-xs font-bold text-white truncate">{auth.user?.name ?? 'Freelancer'}</p>
          <p class="text-[10px] text-emerald-400 flex items-center gap-1 font-medium">{auth.user?.is_verified ? '✓ Verified Worker' : '⏳ Belum Terverifikasi'}</p>
        </div>
      </div>
